    ...
    if (instance) {
        ...
        if (appErrorHandler) {
            ...
            callWithErrorHandling(
                appErrorHandler,
                null,
                10, [err, exposedInstance, errorInfo]
            );
            resetTracking();
            return;
        }
...
function watchPostEffect(effect, options) {
    return doWatch(
        effect,
        null, !!(process.env.NODE_ENV !== "production") ? extend({}, options, {
            flush: "post"
        }) : {
            flush: "post"
        }
    ...
    if (needCastKeys) {
        ...
        for (let i = 0; i < needCastKeys.length; i++) {
            ...
            props[key] = resolvePropValue(
                ...
                key,
                castValues[key],
                instance, !hasOwn(castValues, key)
            );
        }
    }
...
function validateProps(rawProps, props, instance) {
    ...
    for (const key in options) {
        ...
        validateProp(
            key,
            resolvedValues[key],
            opt, !!(process.env.NODE_ENV !== "production") ? shallowReadonly(resolvedValues) : resolvedValues, !hasOwn(rawProps, key) && !hasOwn(rawProps, hyphenate(key))
        );
    }
}
        ...
        if (shapeFlag & 16) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                unmount(
                    ...
                    parentComponent,
                    parentSuspense,
                    shouldRemove, !!child.dynamicChildren
                );
            }
        }
    },
...
function setupStatefulComponent(instance, isSSR) {
    ...
    if (setup) {
        ...
        const setupResult = callWithErrorHandling(
            setup,
            instance,
            0, [!!(process.env.NODE_ENV !== "production") ? shallowReadonly(instance.props) : instance.props,
                setupContext
            ]
        );
        resetTracking();
